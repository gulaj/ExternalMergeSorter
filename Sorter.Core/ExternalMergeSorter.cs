...
namespace Sorter.Core
{
    public class ExternalMergeSorter
    {
        ...
        private DataRecord[] _unsortedRows;
        ...
        public ExternalMergeSorter(ExternalMergeSorterOptions options)
        {
            ...
            _unsortedRows = Array.Empty<DataRecord>();
        }
        ...
        public async Task Sort(Stream source, Stream target, CancellationToken cancellationToken)
        {
            ...
            _unsortedRows = new DataRecord[_maxUnsortedRows];
            if (files.Count == 1)
            {
        ...
        {
            var fileSize = _options.Split.FileChunkSize;
            var buffer = new byte[fileSize];
            var extraBuffer = new List<byte>();
            var filenames = new List<string>();
            var totalFiles = Math.Ceiling(sourceStream.Length / (double)_options.Split.FileChunkSize);
            if (!Directory.Exists(_options.FileLocation))
            {
        ...
        private static async Task<DataRecord> GetDataRecordRowAsync(StreamReader streamReader)
        {
            string dataRow = (await streamReader.ReadLineAsync())!;
            var dataObject = dataRow.Split('.');
            var number = int.Parse(dataObject[0]);
            var text =  dataObject[1].Trim()     ;
            var dataRecord = new DataRecord(number, text);
            return dataRecord;
        }
        private async Task SortFile(Stream unsortedFile, Stream target)
        {
            ...
            while (!streamReader.EndOfStream)
            {
                ...
                _unsortedRows[counter++] = await GetDataRecordRowAsync(streamReader);
            }
            ...
            CancellationToken cancellationToken)
        {
            ...
            while (!done)
            {
                ...
                var valueToWrite = rows[0].Value.ToString();
                ...
                var value = await GetDataRecordRowAsync(streamReaders[streamReaderIndex]);
            ...
            IReadOnlyList<string> sortedFiles)
        {
            ...
            for (var i = 0; i < sortedFiles.Count; i++)
            {
                ...
                var value = await GetDataRecordRowAsync( streamReaders[i]);
                var row = new Row
                {
                    Value = value!,
                    StreamReader = i
